...
{
    public sealed class DataPoint8BitNoSignScaledPercentU8 : DataPoint
    {
        ...
        public override object FromDataPoint(string data, bool isDebug = false)
        {
            ...
            return FromDataPoint(dataConverted, isDebug);
        }
        ...
        {
            return ToDataPoint(float.Parse(value, CultureInfo.InvariantCulture), isDebug);
        }
        ...
        public override byte[] ToDataPoint(object val, bool isDebug = false)
        {
            ...
            else
            {
                if (isDebug)
                {
                    System.Diagnostics.Debug.WriteLine("5.004", "input value received is not a valid type");
                }
                return dataPoint;
            }
            ...
            if (input < 0 || input > 255)
            {
                if (isDebug)
                {
                    System.Diagnostics.Debug.WriteLine("5.004", "input value received is not in a valid range");
                }
                return dataPoint;
            }

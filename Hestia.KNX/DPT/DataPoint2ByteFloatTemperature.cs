...
{
    public sealed class DataPoint2ByteFloatTemperature : DataPoint
    {
        ...
        public override byte[] ToDataPoint(object val, bool isDebug = false)
        {
            ...
            else
            {
                if(isDebug)
                {
                    System.Diagnostics.Debug.WriteLine("9.001", "input value received is not a valid type");
                }
                return dataPoint;
            }
            ...
            if (value < -273 || value > +670760)
            {
                if(isDebug)
                {
                    System.Diagnostics.Debug.WriteLine("9.001", "input value received is not in a valid range");
                }
                return dataPoint;
            }
